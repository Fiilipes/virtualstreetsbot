...
module.exports.run = async (client, interaction) => {
  ...
  const isAssistance = false; // disabled for this version of bot
  const isNoTag = noTagPhrases.includes(tagsInput.trim().toLowerCase());

  const { valid, response } = await gmapsUrltoData(urlInput);

  if (!valid) return channel.createMessage({ content: response });

  const { address, discordEmojiFlag, permalink, date } = response;
  ...
  if (!isAssistance) {
    await interaction
      .createFollowup({ content: 'posted', flags: 64 })
      .then((msg) => {
        msg.delete();
      });
    await channel.createMessage({
      content: `${discordEmojiFlag}${!isNoTag ? ' ' + tagsInput : ''} ${convertDate(date)} in ${address}  •  [Location link](<${pythonMetadata.permalink}>) by ${author.mention}${descriptionInput ? '  •  ' + descriptionInput : ''}`,
      allowedMentions: {
    ...
    await interaction
      ...
      .then((msg) => {
        ...
        const collector = (componentInteraction) => {
          return (
            componentInteraction instanceof ComponentInteraction &&
            componentInteraction.data.custom_id.startsWith(
        ...
        const collectorHandler = (componentInteraction) => {
          if (
            componentInteraction.data.custom_id ===
            `assistance${interaction.member.id}`
          ) {
            // Store the user's selected options
            userSelections[interaction.member.id] =
              componentInteraction.data.values;
            componentInteraction.acknowledge(); // Acknowledge the interaction
          } else if (
            componentInteraction.data.custom_id ===
            `submit_button${interaction.member.id}`
          ) {
            // Handle the button click
            if (userSelections[interaction.member.id]) {
              const selectedOptions = userSelections[interaction.member.id]
                .map((value) => {
                  const option = assistanceSelectMenuTags.find(
                    (tag) => tag.value === value
                  );
                  return option
                    ? `${option.emoji ? `${option.emoji.name} ` : ''}${option.label}`
                    : value;
                })
                .join(', ');
